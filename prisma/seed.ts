...
async function main(): Promise<void> {
  ...
  await prisma.auditLog.createMany({
    data: [
      { invoiceId: inv1.id, userId, action: 'CREATED', toStatus: 'DRAFT' },
      { invoiceId: inv1.id, userId, action: 'VALIDATED', fromStatus: 'DRAFT', toStatus: 'ISSUED' },
      { invoiceId: inv1.id, userId, action: 'EMAIL_SENT', metadata: { to: 'facturation@tunisietelecom.tn' } },
      { invoiceId: inv1.id, userId, action: 'PAID', fromStatus: 'ISSUED', toStatus: 'PAID' },
      { invoiceId: inv2.id, userId, action: 'CREATED', toStatus: 'DRAFT' },
      { invoiceId: inv2.id, userId, action: 'VALIDATED', fromStatus: 'DRAFT', toStatus: 'ISSUED' },
    ],
  });
